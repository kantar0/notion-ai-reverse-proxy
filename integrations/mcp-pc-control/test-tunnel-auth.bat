...
echo.
echo === Health via tunnel (con retry/backoff) ===
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop';
function Invoke-WithRetry([string]$Url,[hashtable]$Headers){
  $max=8; $base=0.6;
  for($i=0;$i -lt $max;$i++){
    try{
      $r=Invoke-WebRequest -UseBasicParsing $Url -Headers $Headers -Method GET;
...
$r=Invoke-WithRetry ('%TUNNELURL%/health') @{}; $r.Content" 

echo.
echo === Auth-check via tunnel CON token (retry/backoff en 429/5xx) ===
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop';
...
$r=Invoke-WithRetry ('%TUNNELURL%/auth-check') $h; $r.Content" 

echo.
echo === MCP initialize via tunnel (retry/backoff) ===
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop';
...
function Invoke-PostWithRetry([string]$Url,[hashtable]$Headers,[string]$Body){
  ...
  for($i=0;$i -lt $max;$i++){
    try{
      $r=Invoke-WebRequest -UseBasicParsing $Url -Headers $Headers -Method POST -Body $Body;
      return $r;
    } catch {
      if($_.Exception.Response){
        $code=[int]$_.Exception.Response.StatusCode.value__;
